...
// ─── 상수 ──────────────────────────────────────────────────────
const CONCEPT_SUBJECT_ID = 205;
const SAVE_KEY = 'concept_industry_progress';
...
// ─── 메인 컴포넌트 ──────────────────────────────────────────────
export default function ConceptIndustryPage() {
    ...
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px', background: '#f8f9fa', minHeight: '100vh' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg,#1e3a5f,#1d4ed8)', color: '#fff', borderRadius: 12, padding: '20px 24px', marginBottom: 24 }}>
          <div style={{ fontSize: '1.3em', fontWeight: 'bold', marginBottom: 4 }}>🚆 철도산업발전기본법 핵심개념 문제</div>
          <div style={{ fontSize: '.85em', opacity: .85 }}>기본계획·구조개혁·PSO 집중 훈련 — {allQuestions.length}문항</div>
          <div style={{ fontSize: '.78em', opacity: .7, marginTop: 6 }}>법령 기준: 철도산업발전기본법</div>
        </div>
        ...
        <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, padding: '12px 16px', marginBottom: 20, fontSize: '.85em', color: '#1e3a8a' }}>
          💡 <strong>핵심개념 문제 안내:</strong> 철도산업발전기본법 조문의 정의·절차·원칙·PSO를 다루는 핵심 훈련 문항입니다.
