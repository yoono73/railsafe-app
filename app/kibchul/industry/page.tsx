...
const SAVE_KEY = 'industry_exam_progress';
const LS_WRONG = 'kibchul_wrong';
const INDUSTRY_SUBJECT_ID = 100;

function saveWrongEntry(q: IndustryQuestion, selected: number) {
  try {
    ...
    const entry = {
      subjectId: INDUSTRY_SUBJECT_ID,
      sessionId: INDUSTRY_PART_LABEL[q.part as Part] ?? `PART ${q.part}`,
      questionId: q.id,
      question: q.question,
      choices: q.choices,
      answer: q.answer,
...
// ─── 메인 컴포넌트 ──────────────────────────────────────────────
export default function IndustryExamPage() {
  ...
  const resumeQuiz = useCallback(() => {
    ...
    const qMap = new Map(INDUSTRY_EXAM.map(q => [q.id, q]));
    const pool = savedProgress.questionIds.map(id => qMap.get(id)).filter(Boolean) as IndustryQuestion[];
    setQuestions(pool);
  ...
  if (mode === 'home') {
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px', background: '#f8f9fa', minHeight: '100vh' }}>
        {/* 헤더 */}
        <div style={{ background: 'linear-gradient(135deg,#312e81,#4338ca)', color: '#fff', borderRadius: 12, padding: '20px 24px', marginBottom: 24 }}>
          <div style={{ fontSize: '1.3em', fontWeight: 'bold', marginBottom: 4 }}>🚄 철도산업발전기본법 CBT</div>
          <div style={{ fontSize: '.85em', opacity: .85 }}>기출·복원 통합문제집 2026 — 현행법 검증본</div>
          <div style={{ fontSize: '.78em', opacity: .7, marginTop: 6 }}>법령 기준: [시행 2026.6.3], 시행령 [시행 2026.1.2]</div>
        </div>
        ...
        <div style={{ background: '#fff8e1', border: '1px solid #f59e0b', borderRadius: 8, padding: '12px 16px', marginBottom: 20, fontSize: '.85em', color: '#78350f' }}>
          ...
          PART 4는 OX 명제를 4지선다로 역변환한 함정 훈련 문항입니다.
        </div>

        {/* 통계 카드 */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 16 }}>
          {(['S', 'A+', 'A'] as FilterGrade[]).map(g => {
            const cnt = INDUSTRY_EXAM.filter(q => q.grade === g).length;
            return (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 24 }}>
          {([1, 2, 3, 4, 5, 6] as Part[]).map(p => {
            ...
            return (
              <div key={p} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '8px 6px', textAlign: 'center' }}>
                <div style={{ fontSize: '.75em', color: '#7c3aed', fontWeight: 'bold', marginBottom: 2 }}>P{p}</div>
                <div style={{ fontSize: '1.2em', fontWeight: 'bold' }}>{cnt}</div>
                <div style={{ fontSize: '.65em', color: '#666', lineHeight: 1.3 }}>{label.split('(')[0].trim()}</div>
              </div>
            );
          })}
        </div>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {(['ALL', 'S', 'A+', 'A'] as FilterGrade[]).map(g => (
                <button key={g} onClick={() => setFilterGrade(g)}
                  ...
                  {g} {g !== 'ALL' ? `(${INDUSTRY_EXAM.filter(q => q.grade === g).length})` : `(${INDUSTRY_EXAM.length})`}
                </button>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            {filterPart !== 0 && (
              <div style={{ fontSize: '.8em', color: '#6b7280', marginTop: 6 }}>
                {INDUSTRY_PART_LABEL[filterPart as Part]}
              </div>
            )}
          </div>
  ...
  if (mode === 'result') {
    ...
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px', background: '#f8f9fa', minHeight: '100vh' }}>
        <div style={{ background: 'linear-gradient(135deg,#312e81,#4338ca)', color: '#fff', borderRadius: 12, padding: '20px 24px', marginBottom: 24, textAlign: 'center' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '12px 14px', marginBottom: 16 }}>
          ...
          {([1, 2, 3, 4] as Part[]).map(p => {
            ...
            return (
              <div key={p} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0', borderBottom: p < 4 ? '1px solid #f3f4f6' : 'none' }}>
                <span style={{ fontSize: '.85em', color: '#555' }}>P{p} {INDUSTRY_PART_LABEL[p].split('(')[0].trim()}</span>
            ...
            : answers.filter(a => !a.correct).map(a => {
              ...
              return (
                <div key={a.qid} style={{ background: '#fff', border: '1px solid #fca5a5', borderRadius: 8, padding: '12px 14px', marginBottom: 10 }}>
                  <div style={{ fontSize: '.8em', color: '#7c3aed', marginBottom: 4 }}>
                    P{q2.part} · {INDUSTRY_PART_LABEL[q2.part as Part].split('(')[0].trim()} · {q2.grade}등급
                  </div>
    ...
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '16px', background: '#f8f9fa', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '20px', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,.06)' }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          ...
          <span style={{ background: isOxPart ? '#7c3aed' : '#f3f4f6', color: isOxPart ? '#fff' : '#555', borderRadius: 4, padding: '2px 8px', fontSize: '.78em' }}>
      ...
      {q.choices.map((c, i) => {
        ...
        if (revealed) {
          ...
          bg = '#ede9fe'; border = '#7c3aed'; color = '#4c1d95'; fontWeight = 'bold';
        }
        return (
      ...
      <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
        {!revealed ? (
          <button onClick={handleReveal} disabled={selected === null}
            style={{ flex: 1, padding: '13px', background: selected === null ? '#d1d5db' : '#4338ca', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 'bold', cursor: selected === null ? 'default' : 'pointer', fontSize: '.95em' }}>
            정답 확인
          </button>
        ) : (
