...
// ─── 메인 컴포넌트 ──────────────────────────────────────────────
export default function ConceptLawPage() {
  ...
  if (mode === 'home') {
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px', background: '#f8f9fa', minHeight: '100vh' }}>
        {/* 헤더 */}
        <div style={{ background: 'linear-gradient(135deg,#14532d,#16a34a)', color: '#fff', borderRadius: 12, padding: '20px 24px', marginBottom: 24 }}>
          <div style={{ fontSize: '1.3em', fontWeight: 'bold', marginBottom: 4 }}>📘 교통안전법 핵심개념 문제</div>
          <div style={{ fontSize: '.85em', opacity: .85 }}>교통안전법·도로교통법·철도안전관련 핵심 훈련 — {allQuestions.length}문항</div>
        ...
        <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 8, padding: '12px 16px', marginBottom: 20, fontSize: '.85em', color: '#14532d' }}>
          💡 <strong>핵심개념 문제 안내:</strong> 교통안전법의 핵심 개념을 다루는 훈련 문항입니다.
        ...
        <button onClick={startQuiz} disabled={allQuestions.length === 0}
          ...
          {allQuestions.length > 0 ? `🚀 ${allQuestions.length}문항 시작` : '문항 준비 중'}
        </button>
      </div>
    );
  }
  ...
  if (mode === 'result') {
    ...
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px', background: '#f8f9fa', minHeight: '100vh' }}>
        <div style={{ background: 'linear-gradient(135deg,#14532d,#16a34a)', color: '#fff', borderRadius: 12, padding: '20px 24px', marginBottom: 24, textAlign: 'center' }}>
