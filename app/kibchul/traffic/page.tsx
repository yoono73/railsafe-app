...
const GRADE_COLOR: Record<string, string> = {
  S: '#7c3aed', 'A+': '#d97706', A: '#2563eb',
};
const SAVE_KEY = 'traffic_exam_progress';
const LS_WRONG = 'kibchul_wrong';
const TRAFFIC_SUBJECT_ID = 200;

function saveWrongEntry(q: TrafficQuestion, selected: number) {
  try {
    ...
    const entry = {
      subjectId: TRAFFIC_SUBJECT_ID,
      sessionId: TRAFFIC_PART_LABEL[q.part as TrafficPart] ?? `PART ${q.part}`,
      questionId: q.id,
      question: q.question,
      choices: q.choices,
      answer: q.answer,
    ...
    saveAttempt({ subject_id: TRAFFIC_SUBJECT_ID, session_id: entry.sessionId, kibchul_qid: q.id, is_correct: false, selected, answer: q.answer }).catch(() => {});
  } catch {}
}
...
// ─── 메인 컴포넌트 ──────────────────────────────────────────────
export default function TrafficExamPage() {
  ...
  const resumeQuiz = useCallback(() => {
    ...
    const pool = savedProgress.questionIds.map(id => qMap.get(id)).filter(Boolean) as TrafficQuestion[];
    setQuestions(pool);
  ...
  const PARTS: TrafficPart[] = [1, 2, 3, 4, 5];
  ...
  if (mode === 'home') {
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px', background: '#f8f9fa', minHeight: '100vh' }}>
        {/* 헤더 */}
        <div style={{ background: 'linear-gradient(135deg,#065f46,#059669)', color: '#fff', borderRadius: 12, padding: '20px 24px', marginBottom: 24 }}>
          <div style={{ fontSize: '1.3em', fontWeight: 'bold', marginBottom: 4 }}>🚦 교통안전법 CBT</div>
          <div style={{ fontSize: '.85em', opacity: .85 }}>기출·복원 통합문제집 2026 — 현행법 검증본</div>
        ...
        <div style={{ background: '#fff8e1', border: '1px solid #f59e0b', borderRadius: 8, padding: '12px 16px', marginBottom: 20, fontSize: '.85em', color: '#78350f' }}>
          ⚠️ <strong>학습용 자료 안내:</strong> 기출·복원 및 법령 원문 기반 재구성 자료입니다.
          수치·벌칙 등은 개정 가능성이 있으므로 <strong>시험 직전 현행 법령을 반드시 재확인</strong>하세요.
          PART 5는 OX 명제를 4지선다 함정 문제로 변환한 훈련 문항입니다.
        </div>

        {/* 등급 통계 카드 */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 16 }}>
          {(['S', 'A+', 'A'] as FilterGrade[]).map(g => {
            const cnt = TRAFFIC_EXAM.filter(q => q.grade === g).length;
            return (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 6, marginBottom: 24 }}>
          {PARTS.map(p => {
            const cnt = TRAFFIC_EXAM.filter(q => q.part === p).length;
            const label = TRAFFIC_PART_LABEL[p];
            return (
              <div key={p} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '8px 4px', textAlign: 'center' }}>
                <div style={{ fontSize: '.7em', color: p === 5 ? '#059669' : '#059669', fontWeight: 'bold', marginBottom: 2 }}>P{p}{p === 5 ? ' OX' : ''}</div>
                <div style={{ fontSize: '1.2em', fontWeight: 'bold' }}>{cnt}</div>
                <div style={{ fontSize: '.6em', color: '#666', lineHeight: 1.2 }}>{label.split('(')[0].trim().substring(0, 8)}</div>
              </div>
            );
          })}
        </div>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {(['ALL', 'S', 'A+', 'A'] as FilterGrade[]).map(g => (
                <button key={g} onClick={() => setFilterGrade(g)}
                  style={{ padding: '5px 14px', borderRadius: 6, border: `2px solid ${filterGrade === g ? (GRADE_COLOR[g] ?? '#059669') : '#d1d5db'}`, background: filterGrade === g ? (GRADE_COLOR[g] ?? '#059669') : '#fff', color: filterGrade === g ? '#fff' : '#374151', fontWeight: filterGrade === g ? 'bold' : 'normal', cursor: 'pointer', fontSize: '.9em' }}>
                  {g} {g !== 'ALL' ? `(${TRAFFIC_EXAM.filter(q => q.grade === g).length})` : `(${TRAFFIC_EXAM.length})`}
                </button>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button onClick={() => setFilterPart(0)}
                style={{ padding: '5px 14px', borderRadius: 6, border: `2px solid ${filterPart === 0 ? '#065f46' : '#d1d5db'}`, background: filterPart === 0 ? '#065f46' : '#fff', color: filterPart === 0 ? '#fff' : '#374151', fontWeight: filterPart === 0 ? 'bold' : 'normal', cursor: 'pointer', fontSize: '.88em' }}>
                전체 ({TRAFFIC_EXAM.length})
              </button>
              {PARTS.map(p => (
                <button key={p} onClick={() => setFilterPart(p)}
                  style={{ padding: '5px 14px', borderRadius: 6, border: `2px solid ${filterPart === p ? '#065f46' : '#d1d5db'}`, background: filterPart === p ? '#065f46' : '#fff', color: filterPart === p ? '#fff' : '#374151', fontWeight: filterPart === p ? 'bold' : 'normal', cursor: 'pointer', fontSize: '.88em' }}>
                  P{p}{p === 5 ? '(OX)' : ''} ({TRAFFIC_EXAM.filter(q => q.part === p).length})
                </button>
              ))}
            </div>
            {filterPart !== 0 && (
              <div style={{ fontSize: '.8em', color: '#6b7280', marginTop: 6 }}>
                {TRAFFIC_PART_LABEL[filterPart as TrafficPart]}
              </div>
            )}
          </div>
        ...
        <button onClick={startQuiz} disabled={filtered.length === 0}
          style={{ width: '100%', padding: '14px', background: filtered.length === 0 ? '#d1d5db' : '#059669', color: '#fff', border: 'none', borderRadius: 10, fontSize: '1.05em', fontWeight: 'bold', cursor: filtered.length === 0 ? 'default' : 'pointer' }}>
  ...
  if (mode === 'result') {
    ...
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px', background: '#f8f9fa', minHeight: '100vh' }}>
        <div style={{ background: 'linear-gradient(135deg,#065f46,#059669)', color: '#fff', borderRadius: 12, padding: '20px 24px', marginBottom: 24, textAlign: 'center' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '12px 14px', marginBottom: 16 }}>
          ...
          {PARTS.map(p => {
            ...
            return (
              <div key={p} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0', borderBottom: p < 5 ? '1px solid #f3f4f6' : 'none' }}>
                <span style={{ fontSize: '.85em', color: '#555' }}>P{p} {TRAFFIC_PART_LABEL[p].split('(')[0].trim().substring(0, 14)}</span>
        ...
        <div style={{ marginTop: 20 }}>
          ...
          {answers.filter(a => !a.correct).length === 0
            ...
            : answers.filter(a => !a.correct).map(a => {
              ...
              const isOxP = q2.part === 5;
              return (
                <div key={a.qid} style={{ background: '#fff', border: '1px solid #fca5a5', borderRadius: 8, padding: '12px 14px', marginBottom: 10 }}>
                  <div style={{ fontSize: '.8em', color: '#059669', marginBottom: 4 }}>
                    P{q2.part}{isOxP ? ' · OX함정' : ' · ' + TRAFFIC_PART_LABEL[q2.part].split('(')[0].trim().substring(0, 10)} · {q2.grade}등급
                  </div>
                  <div style={{ fontSize: '.9em', marginBottom: 8, fontWeight: 500 }}>{q2.question}</div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          ...
          <button onClick={startQuiz}
            style={{ flex: 1, padding: '12px', background: '#059669', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 'bold', cursor: 'pointer' }}>
            다시 풀기
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '16px', background: '#f8f9fa', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#e5e7eb', borderRadius: 99, height: 6, marginBottom: 16 }}>
        <div style={{ background: '#059669', height: 6, borderRadius: 99, width: `${progress}%`, transition: 'width .3s' }} />
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '20px', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,.06)' }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
          ...
          <span style={{ background: isOxPart ? '#059669' : '#f3f4f6', color: isOxPart ? '#fff' : '#555', borderRadius: 4, padding: '2px 8px', fontSize: '.78em' }}>
            {isOxPart ? '⚡OX함정' : `P${q.part}`}
          </span>
      ...
      {q.choices.map((c, i) => {
        ...
        if (revealed) {
          ...
          bg = '#ecfdf5'; border = '#059669'; color = '#065f46'; fontWeight = 'bold';
        }
        return (
      ...
      {revealed && (
        <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 8, padding: '12px 14px', marginTop: 8, fontSize: '.88em', color: '#14532d' }}>
          ...
          {q.ref && (
            <div style={{ fontSize: '.82em', color: '#6b7280', marginTop: 6 }}>
              📖 근거: {q.ref}
            </div>
          )}
        </div>
      ...
      <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
        {!revealed ? (
          <button onClick={handleReveal} disabled={selected === null}
            style={{ flex: 1, padding: '13px', background: selected === null ? '#d1d5db' : '#059669', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 'bold', cursor: selected === null ? 'default' : 'pointer', fontSize: '.95em' }}>
            정답 확인
          </button>
        ) : (
          <button onClick={handleNext}
            style={{ flex: 1, padding: '13px', background: '#047857', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 'bold', cursor: 'pointer', fontSize: '.95em' }}>
