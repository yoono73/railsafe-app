...
// ─── 상수 ──────────────────────────────────────────────────────
const CONCEPT_SUBJECT_ID = 3;
const SAVE_KEY = 'concept_operation_progress';
...
// ─── 메인 컴포넌트 ──────────────────────────────────────────────
export default function ConceptOperationPage() {
  ...
  const handleReveal = () => {
    ...
    else removeAttempt(`concept_op_${q.id}`).catch(() => {});
  };
  ...
  if (mode === 'home') {
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px', background: '#f8f9fa', minHeight: '100vh' }}>
        {/* 헤더 */}
        <div style={{ background: 'linear-gradient(135deg,#0f4c2e,#16a34a)', color: '#fff', borderRadius: 12, padding: '20px 24px', marginBottom: 24 }}>
          <div style={{ fontSize: '1.3em', fontWeight: 'bold', marginBottom: 4 }}>🚂 열차운전 핵심개념 문제</div>
          <div style={{ fontSize: '.85em', opacity: .85 }}>운전규칙·신호·폐색·비상조치 집중 훈련 — {allQuestions.length}문항</div>
          <div style={{ fontSize: '.78em', opacity: .7, marginTop: 6 }}>법령 기준: 철도차량운전규칙 [시행 2021.10.26]</div>
        </div>
        ...
        {/* 안내 */}
        <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 8, padding: '12px 16px', marginBottom: 20, fontSize: '.85em', color: '#14532d' }}>
          💡 <strong>핵심개념 문제 안내:</strong> 철도차량운전규칙·도시철도운전규칙의 정의·조건·절차와 신호·폐색·비상조치를 다루는 핵심 훈련 문항입니다.
          선지별 해설과 함정 포인트가 함께 제공됩니다.
        </div>

        {/* 문항 수 카드 */}
        <div style={{ background: '#fff', border: '2px solid #16a34a', borderRadius: 10, padding: '16px 20px', marginBottom: 20, textAlign: 'center' }}>
          <div style={{ fontSize: '2em', fontWeight: 'bold', color: '#15803d' }}>{allQuestions.length}</div>
        ...
        <button onClick={startQuiz} disabled={allQuestions.length === 0}
          style={{ width: '100%', padding: '14px', background: allQuestions.length === 0 ? '#d1d5db' : '#16a34a', color: '#fff', border: 'none', borderRadius: 10, fontSize: '1.05em', fontWeight: 'bold', cursor: allQuestions.length === 0 ? 'default' : 'pointer' }}>
  ...
  if (mode === 'result') {
    ...
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px', background: '#f8f9fa', minHeight: '100vh' }}>
        <div style={{ background: 'linear-gradient(135deg,#0f4c2e,#16a34a)', color: '#fff', borderRadius: 12, padding: '20px 24px', marginBottom: 24, textAlign: 'center' }}>
            ...
            : answers.filter(a => !a.correct).map(a => {
              ...
              return (
                <div key={a.qid} style={{ background: '#fff', border: '1px solid #fca5a5', borderRadius: 8, padding: '12px 14px', marginBottom: 10 }}>
                  <div style={{ fontSize: '.78em', color: '#15803d', marginBottom: 4 }}>{q2.exam_area}</div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          ...
          <button onClick={startQuiz}
            style={{ flex: 1, padding: '12px', background: '#16a34a', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 'bold', cursor: 'pointer' }}>
            다시 풀기
          </button>
        </div>
      </div>
    ...
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '16px', background: '#f8f9fa', minHeight: '100vh' }}>
      ...
      {q.choices.map((c, i) => {
        ...
        if (revealed) {
          ...
          bg = '#f0fdf4'; border = '#16a34a'; color = '#14532d'; fontWeight = 'bold';
        }
        return (
          <div key={idx}>
      ...
      <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
        {!revealed ? (
          <button onClick={handleReveal} disabled={selected === null}
            style={{ flex: 1, padding: '13px', background: selected === null ? '#d1d5db' : '#16a34a', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 'bold', cursor: selected === null ? 'default' : 'pointer', fontSize: '.95em' }}>
            정답 확인
          </button>
        ) : (
          <button onClick={handleNext}
            style={{ flex: 1, padding: '13px', background: '#15803d', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 'bold', cursor: 'pointer', fontSize: '.95em' }}>
