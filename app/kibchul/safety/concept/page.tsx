...
// ─── 상수 ──────────────────────────────────────────────────────
const CONCEPT_SUBJECT_ID = 207;
const SAVE_KEY = 'concept_safety_progress';
...
// ─── 메인 컴포넌트 ──────────────────────────────────────────────
export default function ConceptSafetyPage() {
  ...
  if (mode === 'home') {
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px', background: '#f8f9fa', minHeight: '100vh' }}>
        {/* 헤더 */}
        <div style={{ background: 'linear-gradient(135deg,#1e3a5f,#0f766e)', color: '#fff', borderRadius: 12, padding: '20px 24px', marginBottom: 24 }}>
          <div style={{ fontSize: '1.3em', fontWeight: 'bold', marginBottom: 4 }}>🚂 철도안전법 핵심개념 문제</div>
          <div style={{ fontSize: '.85em', opacity: .85 }}>핵심 조문·수치·절차·제재 집중 훈련 — {allQuestions.length}문항</div>
          <div style={{ fontSize: '.78em', opacity: .7, marginTop: 6 }}>법령 기준: [시행 2026.3.3] 법률 제21188호</div>
        </div>

        {/* 이어풀기 배너 */}
        {savedProgress && (
          <div style={{ background: '#f0fdfa', border: '2px solid #14b8a6', borderRadius: 10, padding: '14px 18px', marginBottom: 20 }}>
            <div style={{ fontWeight: 'bold', color: '#0f766e', marginBottom: 6 }}>📌 이어서 풀기 가능</div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={resumeQuiz}
                style={{ flex: 2, padding: '10px', background: '#0f766e', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 'bold', cursor: 'pointer', fontSize: '.92em' }}>
        ...
        {/* 안내 */}
        <div style={{ background: '#f0fdfa', border: '1px solid #99f6e4', borderRadius: 8, padding: '12px 16px', marginBottom: 20, fontSize: '.85em', color: '#134e4a' }}>
          💡 <strong>핵심개념 문제 안내:</strong> 철도안전법 조문의 정의·수치·절차·제재를 다루는 핵심 훈련 문항입니다.
          선지별 해설과 함정 포인트가 함께 제공됩니다.
        </div>

        {/* 문항 수 카드 */}
        <div style={{ background: '#fff', border: '2px solid #0d9488', borderRadius: 10, padding: '16px 20px', marginBottom: 20, textAlign: 'center' }}>
          <div style={{ fontSize: '2em', fontWeight: 'bold', color: '#0f766e' }}>{allQuestions.length}</div>
        ...
        <button onClick={startQuiz} disabled={allQuestions.length === 0}
          style={{ width: '100%', padding: '14px', background: allQuestions.length === 0 ? '#d1d5db' : '#0f766e', color: '#fff', border: 'none', borderRadius: 10, fontSize: '1.05em', fontWeight: 'bold', cursor: allQuestions.length === 0 ? 'default' : 'pointer' }}>
  ...
  if (mode === 'result') {
    ...
    return (
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '24px 16px', background: '#f8f9fa', minHeight: '100vh' }}>
        <div style={{ background: 'linear-gradient(135deg,#1e3a5f,#0f766e)', color: '#fff', borderRadius: 12, padding: '20px 24px', marginBottom: 24, textAlign: 'center' }}>
            ...
            : answers.filter(a => !a.correct).map(a => {
              ...
              return (
                <div key={a.qid} style={{ background: '#fff', border: '1px solid #fca5a5', borderRadius: 8, padding: '12px 14px', marginBottom: 10 }}>
                  <div style={{ fontSize: '.78em', color: '#0f766e', marginBottom: 4 }}>{q2.exam_area}</div>
                  ...
                  {q2.memory_line && (
                    <div style={{ fontSize: '.8em', color: '#134e4a', background: '#f0fdfa', borderRadius: 4, padding: '4px 8px', marginTop: 4 }}>
                      🔑 {q2.memory_line}
                    </div>
                  )}
                </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          ...
          <button onClick={startQuiz}
            style={{ flex: 1, padding: '12px', background: '#0f766e', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 'bold', cursor: 'pointer' }}>
            다시 풀기
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '16px', background: '#f8f9fa', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#e5e7eb', borderRadius: 99, height: 6, marginBottom: 16 }}>
        <div style={{ background: '#0d9488', height: 6, borderRadius: 99, width: `${progress}%`, transition: 'width .3s' }} />
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '20px', marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,.06)' }}>
        <div style={{ marginBottom: 10 }}>
          <span style={{ background: '#f0fdfa', color: '#0f766e', borderRadius: 4, padding: '2px 8px', fontSize: '.78em', fontWeight: 'bold' }}>
            {q.exam_area}
          </span>
        </div>
      ...
      {q.choices.map((c, i) => {
        ...
        if (revealed) {
          ...
          bg = '#f0fdfa'; border = '#0d9488'; color = '#134e4a'; fontWeight = 'bold';
        }
        return (
          <div key={idx}>
      ...
      {revealed && (
        <div style={{ background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 8, padding: '12px 14px', marginTop: 4, fontSize: '.88em', color: '#14532d' }}>
          ...
          {q.memory_line && (
            <div style={{ background: '#f0fdfa', border: '1px solid #99f6e4', borderRadius: 6, padding: '6px 10px', marginTop: 6, color: '#134e4a' }}>
      ...
      <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
        {!revealed ? (
          <button onClick={handleReveal} disabled={selected === null}
            style={{ flex: 1, padding: '13px', background: selected === null ? '#d1d5db' : '#0f766e', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 'bold', cursor: selected === null ? 'default' : 'pointer', fontSize: '.95em' }}>
            정답 확인
          </button>
        ) : (
          <button onClick={handleNext}
            style={{ flex: 1, padding: '13px', background: '#0d9488', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 'bold', cursor: 'pointer', fontSize: '.95em' }}>
